...
import AddAmbulance from "../modal/AddAmbulance";
...
export const Ambulance = () => {
    ...
    const { stateLoading, ambulances } = useAmbulance();

    const { slice, pages, count } = useTable(ambulances, page, perPage);
    ...
    const handleOpenAddAmbulance = useCallback(() => {
        setOpenAddAmbulance(true);
    }, []);

    const handleCloseAmbulance = useCallback(() => {
        setOpenAddAmbulance(false);
    }, []);
            ...
            <Box maxH={"91%"} overflowY={"scroll"}>
                <Box width={"full"} bg={"white"}>
                    <Box display={"flex"} justifyContent={"space-between"} alignItems={"center"} px={14} py={3}>
                        <Text className="text-primary_color font-semibold text-3xl">Ambulances</Text>
                        <Center>
                            <CustomButton
                                type="button"
                                text="Add Ambulance Service"
                                variant={"solid"}
                                width={"200px"}
                                fontSize={"16px"}
                                handleClick={handleOpenAddAmbulance}
                            />
                        </Center>
                    </Box>
                </Box>
                    ...
                    <Box my={6} bg={"white"} py={4} px={8} display={"flex"} justifyContent={"space-between"} alignItems={"center"}>
                        ...
                        >
                            <Box display={"flex"} alignItems={"center"} gap={3}>
                                <input
                                    placeholder="Type to search Ambulance"
                            ...
                            {slice?.filter((ambulance) => {
                                ...
                                return (
                                    <tr key={index} className="h-20 border-b">
                                        ...
                                        {/* actions table */}
                                        <td>
                                            <Link to={`/ambulance/${data?._id}`} className="flex justify-center">
